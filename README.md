# EventHive: Event Budget Tracker

> Financial planning and variance tracking with industry-standard cost categories, budget/actual/variance columns, RAG status per line item, scenario modelling, PO & invoice tracking, and CSV/PDF export.
...
- Industry-standard cost categories pre-loaded
- Budget / Actual / Variance columns with auto-calculation
- RAG (Red/Amber/Green) status per line item
- Scenario modelling: Best Case, Base Case, Downturn
- PO and invoice tracking with payment status
- Key ratio dashboard (venue %, tech %, staffing %)
...
|------|-------|-------------|
| Browser | localStorage (`eventhive_event_{eventSlug}_budget`) | Until browser data cleared |
| EventHive | Cloud database (PostgreSQL) | Synced across devices |

This tool stores all data locally in `eventhive_event_{eventSlug}_budget`. Data persists until browser storage is cleared.

---

## Related Tools

- [Sponsor Deliverables Tracker](https://github.com/Visual-Hive/eventhive-tool-sponsor-deliverables) — receives payment data bridge
- [Exhibitor Manager](https://github.com/Visual-Hive/eventhive-tool-exhibitor-manager) — receives stand revenue bridge
...
```bash
# 1. Clone this repo
git clone https://github.com/Visual-Hive/eventhive-tool-budget-tracker.git
cd eventhive-tool-budget-tracker
...
| `TOOL_TITLE` | `Event Budget Tracker` | Title shown inside the tool |
